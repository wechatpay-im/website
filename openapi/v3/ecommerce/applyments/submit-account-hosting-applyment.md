---
title: 提交申请单
description: 接口适用场景：电商平台，可使用该接口，帮助其二级商户进件成为微信支付商户。本接口不支持进件境外个人小微商户。
---
...
| business_time {data-indent=2} | string {data-tooltip=使用json_encode后的字符串 data-encoded=by-json_encode} | 营业期限`JSON`格式字符串
...
| id_card_copy {data-required data-indent=2} | string | 身份证人像面照片
| id_card_national {data-required data-indent=2} | string | 身份证国徽面照片
| id_card_name {data-required data-indent=2} | string {data-tooltip=微信支付公钥/平台证书加密后的BASE64字符串 data-encrypted=by-rsa-pubkey} | 身份证姓名
| id_card_number {data-required data-indent=2} | string {data-tooltip=微信支付公钥/平台证书加密后的BASE64字符串 data-encrypted=by-rsa-pubkey} | 身份证号码
| id_card_valid_time_begin {data-required data-indent=2} | string | 身份证有效期开始时间
| id_card_valid_time {data-required data-indent=2} | string | 身份证有效期结束时间
| id_doc_info {data-indent=1} | object {data-tooltip="对应PHP的array"} | 经营者/法人其他类型证件信息
| id_doc_name {data-required data-indent=2} | string {data-tooltip=微信支付公钥/平台证书加密后的BASE64字符串 data-encrypted=by-rsa-pubkey} | 证件姓名
| id_doc_number {data-required data-indent=2} | string {data-tooltip=微信支付公钥/平台证书加密后的BASE64字符串 data-encrypted=by-rsa-pubkey} | 证件号码
