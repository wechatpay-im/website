---
title: 开具旅客运输行业电子发票
description: 商户完成收款后，调用本接口开具旅客运输行业电子发票并插入微信用户的卡包。请求的交易信息必须真实存在。注意：本接口成功返回仅代表开票请求已被受理，当开票完成时，微信支付会根据商户配置的回调地址进行回调通知，商户也可以通过【查询电子发票】接口获取开票结果及开票信息。蓝字发票开具失败时，可以通过更换fapiao_information.fapiao_id，重新填写发票信息进行重试。每个申请单可至多重试4次。
---
...
| passenger_information {data-indent=3} | object {data-tooltip="对应PHP的array"} | 出行人额外信息
| name {data-required data-indent=4} | string | 出行人名称
| certificate_type {data-required data-indent=4} | string | 出行人证件类型<br/>`IDENTITY_CARD` \| `PASSPORT` \| `HONG_KONG_PERMIT` \| `MACAO_PERMIT` \| `TAIWAN_PERMIT` \| `FOREIGNER_RESIDENCE_PERMIT` \| `HONG_KONG_RESIDENT_CARD` \| `MACAO_RESIDENT_CARD` \| `TAIWAN_RESIDENT_CARD` \| `MILITARY_OFFICER_CARD` \| `ARMED_POLICE_OFFICER_CARD` \| `SOLDIER_CARD` \| `HOMECOMING_CERT` \| `TAIWAN_COMPATRIOT_CERT` 枚举值之一
| certificate_number {data-required data-indent=4} | string {data-tooltip=微信支付公钥/平台证书加密后的BASE64字符串 data-encrypted=by-rsa-pubkey} | 证件号码
| departure_date {data-required data-indent=4} | string | 出行日期
| departure_place {data-required data-indent=4} | string | 出发地
| destination {data-required data-indent=4} | string | 目的地
| transportation_type {data-required data-indent=4} | string | 交通工具类型<br/>`LONG_DISTANCE_BUS` \| `PUBLIC_TRANSPORTATION` \| `CAR` \| `SHIP` \| `OTHER_TRANSPORTATION` 枚举值之一
| transportation_classes {data-indent=4} | string | 交通工具等级<br/>`SHIP_FIRST_CLASS_CABIN` \| `SHIP_SECOND_CLASS_CABIN` \| `SHIP_THIRD_CLASS_CABIN` 枚举值之一
...
```php [异步纯链式]
$instance->v3->newTaxControlFapiao->fapiaoApplications->issuePassengerTransport->postAsync([
  'json' => [
...
```php [异步声明式]
$instance->chain('v3/new-tax-control-fapiao/fapiao-applications/issue-passenger-transport')->postAsync([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [异步属性式]
$instance['v3/new-tax-control-fapiao/fapiao-applications/issue-passenger-transport']->postAsync([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步纯链式]
$response = $instance->v3->newTaxControlFapiao->fapiaoApplications->issuePassengerTransport->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步声明式]
$response = $instance->chain('v3/new-tax-control-fapiao/fapiao-applications/issue-passenger-transport')->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步属性式]
$response = $instance['v3/new-tax-control-fapiao/fapiao-applications/issue-passenger-transport']->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4025863376)
