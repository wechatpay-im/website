---
title: 开具成品油行业电子发票
description: 商户完成收款后，调用本接口开具成品油行业电子发票并插入微信用户的卡包。请求的交易信息必须真实存在。  注意：本接口成功返回仅代表开票请求已被受理，当开票完成时，微信支付会根据商户配置的回调地址进行回调通知，商户也可以通过【查询电子发票】接口获取开票结果及开票信息。蓝字发票开具失败时，可以通过更换fapiao_information.fapiao_id，重新填写发票信息进行重试。每个申请单可至多重试4次。
---
...
| unit {data-indent=3} | string | 成品油单位<br/>`TON` \| `LETER` 枚举值之一
...
| refined_oil_information {data-indent=3} | object {data-tooltip="对应PHP的array"} | 成品油行业额外信息
| station {data-required data-indent=4} | string | 成品油站点
| trade_time {data-required data-indent=4} | string | 交易时间
| gun_number {data-required data-indent=4} | string | 加油枪号码
...
```php [异步纯链式]
$instance->v3->newTaxControlFapiao->fapiaoApplications->issueRefinedOil->postAsync([
  'json' => [
...
```php [异步声明式]
$instance->chain('v3/new-tax-control-fapiao/fapiao-applications/issue-refined-oil')->postAsync([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [异步属性式]
$instance['v3/new-tax-control-fapiao/fapiao-applications/issue-refined-oil']->postAsync([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步纯链式]
$response = $instance->v3->newTaxControlFapiao->fapiaoApplications->issueRefinedOil->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步声明式]
$response = $instance->chain('v3/new-tax-control-fapiao/fapiao-applications/issue-refined-oil')->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步属性式]
$response = $instance['v3/new-tax-control-fapiao/fapiao-applications/issue-refined-oil']->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
