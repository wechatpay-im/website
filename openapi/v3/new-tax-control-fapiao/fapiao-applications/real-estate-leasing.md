---
title: 开具不动产租赁行业电子发票
description: 商户完成收款后，调用本接口开具不动产租赁行业电子发票并插入微信用户的卡包。请求的交易信息必须真实存在。 注意：本接口成功返回仅代表开票请求已被受理，当开票完成时，微信支付会根据商户配置的回调地址进行回调通知，商户也可以通过【查询电子发票】接口获取开票结果及开票信息。
---
...
| real_estate_leasing_information {data-indent=3} | object {data-tooltip="对应PHP的array"} | 不动产租赁特定行业额外信息
| province {data-required data-indent=4} | string | 不动产坐落省份
| city {data-indent=4} | string | 不动产坐落城市
| address {data-required data-indent=4} | string | 不动产坐落地址（详细地址）
| lease_period_start {data-required data-indent=4} | string | 租赁期起
| lease_period_end {data-required data-indent=4} | string | 租赁期止
| cross_city_flag {data-required data-indent=4} | boolean | 跨地（市）标志
| certificate_no {data-indent=4} | string {data-tooltip=微信支付公钥/平台证书加密后的BASE64字符串 data-encrypted=by-rsa-pubkey} | 产权证书/不动产权证号
| plate_no {data-indent=4} | string[] | 车牌号
...
```php [异步纯链式]
$instance->v3->newTaxControlFapiao->fapiaoApplications->realEstateLeasing->postAsync([
  'json' => [
...
```php [异步声明式]
$instance->chain('v3/new-tax-control-fapiao/fapiao-applications/real-estate-leasing')->postAsync([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [异步属性式]
$instance['v3/new-tax-control-fapiao/fapiao-applications/real-estate-leasing']->postAsync([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步纯链式]
$response = $instance->v3->newTaxControlFapiao->fapiaoApplications->realEstateLeasing->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步声明式]
$response = $instance->chain('v3/new-tax-control-fapiao/fapiao-applications/real-estate-leasing')->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
```php [同步属性式]
$response = $instance['v3/new-tax-control-fapiao/fapiao-applications/real-estate-leasing']->post([
  'json' => [
    'sub_mchid'          => '1900000109',
    'fapiao_apply_id'    => '4200000444201910177461284488',
    'buyer_information'  => [
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4015941740)
