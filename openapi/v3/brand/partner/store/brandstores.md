...
| brand_id {data-required data-indent=1} | string | 品牌ID
| store_state {data-indent=1} | string | 门店状态<br/>`OPEN` \| `CREATING` \| `CLOSED` 枚举值之一
| offset {data-indent=1} | integer | 分页起始位置
| limit {data-indent=1} | integer | 分页条数
...
```php [异步纯链式]
$instance->v3->brand->partner->store->brandstores->getAsync([
  'query' => [
    'brand_id'    => '120344',
    'store_state' => 'OPEN',
    'offset'      => 100,
...
```php [异步声明式]
$instance->chain('v3/brand/partner/store/brandstores')->getAsync([
  'query' => [
    'brand_id'    => '120344',
    'store_state' => 'OPEN',
    'offset'      => 100,
...
```php [同步纯链式]
$response = $instance->v3->brand->partner->store->brandstores->get([
  'query' => [
    'brand_id'    => '120344',
    'store_state' => 'OPEN',
    'offset'      => 100,
...
```php [同步声明式]
$response = $instance->chain('v3/brand/partner/store/brandstores')->get([
  'query' => [
    'brand_id'    => '120344',
    'store_state' => 'OPEN',
    'offset'      => 100,
...
| limit | integer | 分页条数
| total_count | integer | 门店总数
| data | object[] {data-tooltip="对应PHP的array"} | 品牌门店列表
| brand_id {data-required data-indent=1} | string | 品牌ID
| store_id {data-indent=1} | string | 品牌门店ID
| store_state {data-indent=1} | string | 门店状态<br/>`OPEN` \| `CREATING` \| `CLOSED` 枚举值之一
| audit_state {data-indent=1} | string | 审核状态<br/>`SUCCESS` \| `PROCESSING` \| `REJECTED` 枚举值之一
| review_reject_reason {data-indent=1} | string | 审核失败原因
| store_basics {data-indent=1} | object {data-tooltip="对应PHP的array"} | 门店基础信息
| store_reference_id {data-indent=2} | string | 商家门店编号
| branch_name {data-indent=2} | string | 门店名称
...
| recipient_state {data-indent=2} | string | 收款绑定状态<br/>`CONFIRMED` \| `ADMIN_REJECTED` \| `CONFIRMING` \| `TIMEOUT_REJECTED` 枚举值之一

{.im-table #response}

参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4016756674)
