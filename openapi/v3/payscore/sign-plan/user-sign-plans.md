...
| plan_id {data-indent=1} | string | 支付分计划ID
| appid {data-indent=1} | string | AppID
| openid {data-indent=1} | string | 待签约用户在商户应用下的OpenID
| merchant_sign_plan_no {data-indent=1} | string | 商户签约计划单号
| notify_url {data-indent=1} | string | 签约成功事件通知回调地址
| sign_plan_detail {data-indent=1} | object[] | 签约计划对应的计划详情列表的商户侧单号信息
| plan_detail_no {data-indent=2} | number | 计划明细序号
| merchant_plan_detail_no {data-indent=2} | string | 商户侧计划明细使用订单号
...
| sign_plan_id {data-indent=1} | string | 计划签约ID
| openid {data-indent=1} | string | 签约计划用户应用下的OpenID
| service_id {data-indent=1} | string | 计划所属服务
| mchid {data-indent=1} | string | 商户号
| appid {data-indent=1} | string | AppID
| merchant_sign_plan_no {data-indent=1} | string | 商户侧签约计划号
| merchant_callback_url {data-indent=1} | string | 商户回调地址
| plan_id {data-indent=1} | string | 签约所属计划ID
| going_detail_no {data-indent=1} | number | 目前用户进行到的计划详情序号
| sign_state {data-indent=1} | string | 计划签约状态
...
| plan_name {data-indent=1} | string | 签约计划的名称
| plan_over_time {data-indent=1} | string | 签约计划的过期时间
| total_origin_price {data-indent=1} | number | 签约计划原总金额(单位分)
| deduction_quantity {data-indent=1} | number | 签约计划扣费次数
| total_actual_price {data-indent=1} | number | 签约计划实际总金额（单位分）
| signed_detail_list {data-indent=1} | object[] | 签约计划明细列表
| plan_detail_no {data-indent=2} | number | 计划明细序号
...
| plan_detail_state {data-indent=2} | string | 计划明细状态
| order_id {data-indent=2} | string | 计划明细对应的支付分服务单号
| merchant_plan_detail_no {data-indent=2} | string | 商户侧计划明细使用订单号
| plan_detail_name {data-indent=2} | string | 计划详情名称
