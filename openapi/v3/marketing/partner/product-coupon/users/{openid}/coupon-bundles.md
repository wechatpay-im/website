---
title: 向用户发放商品券批次组
description: 服务商可以通过本接口向用户发放指定商品券批次组，能否发放受限于商品券组内批次的发放限额：. 商品券批次总预算; 2. 商品券批次每日预算（如果有）; 3. 商品券批次每人限领（如果有）
---
...
| coupon_tag_list {data-indent=2} | string[] | 用户商品券标签列表
...
```php [异步纯链式]
$instance->v3->marketing->partner->productCoupon->users->_openid_->couponBundles->postAsync([
...
```php [异步声明式]
$instance->chain('v3/marketing/partner/product-coupon/users/{openid}/coupon-bundles')->postAsync([
  'openid' => 'oh-394z-6CGkNoJrsDLTTUKiAnp4',
  'json' => [
    'brand_id'          => '120344',
    'product_coupon_id' => '1000000013',
...
```php [异步属性式]
$instance['v3/marketing/partner/product-coupon/users/{openid}/coupon-bundles']->postAsync([
  'openid' => 'oh-394z-6CGkNoJrsDLTTUKiAnp4',
  'json' => [
    'brand_id'          => '120344',
    'product_coupon_id' => '1000000013',
...
```php [同步纯链式]
$response = $instance->v3->marketing->partner->productCoupon->users->_openid_->couponBundles->post([
  'openid' => 'oh-394z-6CGkNoJrsDLTTUKiAnp4',
  'json' => [
    'brand_id'          => '120344',
    'product_coupon_id' => '1000000013',
...
```php [同步声明式]
$response = $instance->chain('v3/marketing/partner/product-coupon/users/{openid}/coupon-bundles')->post([
  'openid' => 'oh-394z-6CGkNoJrsDLTTUKiAnp4',
  'json' => [
    'brand_id'          => '120344',
    'product_coupon_id' => '1000000013',
...
```php [同步属性式]
$response = $instance['v3/marketing/partner/product-coupon/users/{openid}/coupon-bundles']->post([
  'openid' => 'oh-394z-6CGkNoJrsDLTTUKiAnp4',
  'json' => [
    'brand_id'          => '120344',
    'product_coupon_id' => '1000000013',
...
| send_request_no {data-required data-indent=1} | string | 发券请求单号
| send_channel {data-required data-indent=1} | string | 发券渠道<br/>`API` \| `BRAND_MANAGE` \| `MERCHANT_CARD` \| `MEMBER` \| `SMALL_ACTIVITY` \| `RECEIVE_COMPONENT` 枚举值之一
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4016280664)
