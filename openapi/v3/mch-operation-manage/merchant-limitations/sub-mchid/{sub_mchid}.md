...
| recover_way {data-indent=1} | string | 商户被该原因管控的解脱路径<br/>`IRRECOVERABLE` \| `MODIFY_SUBJECT_INFORMATION` \| `MODIFY_SETTLE_ACCOUNT_INFORMATION` \| `VERIFY_INACTIVE_MERCHANT_IDENTITY` \| `SUBMIT_OFFLINE_BUSINESS_SCENARIO_INFORMATION` \| `SUBMIT_INFORMATION_FOR_APPEAL` \| `RESOLVE_TRANSACTION_DISPUTES` \| `MODIFY_ADMINISTRATOR_INFORMATION` \| `CALL_CUSTOMER_SERVICE_AT_95017` \| `UPDATE_BUSINESS_SCENARIO_INFORMATION` \| `SUBMIT_CDD_INFORMATION` \| `WAITING_FOR_PLATFORM_REVIEW` \| `SUBMIT_UBO_INFORMATION` \| `SIGN_ANTI_FRAUD_PLEDGE_AND_VERIFY_FACE` \| `CONTACT_APPROPRIATE_AUTHORITY_FOR_CONSULTATION` \| `MODIFY_ABBREVIATION_INFORMATION` \| `CONFIRM_BUSINESS_TYPE` \| `SUBMIT_ONLINE_BUSINESS_SCENARIO_INFORMATION` \| `CONTACT_SERVICE_PROVIDER` 枚举值之一
