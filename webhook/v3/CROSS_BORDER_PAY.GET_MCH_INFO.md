---
title: 外包内用-查询收款二维码信息(CROSS_BORDER_PAY.GET_MCH_INFO)回调(JSON)
description: 境外钱包用户扫码时，微信支付回调从业机构，查询收款二维码对应信息，用于支付页展示。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`CROSS_BORDER_PAY.GET_MCH_INFO` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型<br/>`encrypt-resource` 枚举值
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`cross_border_pay` 枚举值
...
| acq_tid {data-required data-indent=3} | string | 受理网络交易流水号
| bank_type {data-required data-indent=3} | string | 钱包编码
| qr_cd_mp {data-required data-indent=3} | string | 收款码，传递钱包网络扫码解析的url
...
::: warning :bangbang: {.im-special}

微信支付收到响应后的验签规则为，从响应头取`Authorization`的值，其格式为 `WECHATPAY2-SHA256-RSA2048 mchid="${mchid}", nonce_str="${nonce}", signature="${signature}", timestamp="${timestamp}", serial_no="${serial}"`，详细见下文链接。
...
| headers | object {data-tooltip="对应PHP的Array"} | 应答的头参数，需从业机构签名
| Authorization {data-required data-indent=1} | string | 响应签名值
| body | object {data-tooltip="对应PHP的Array"} | 应答的`JSON`数据结构
| code {data-required data-indent=1} | string | 业务处理状态码<br/>`SUCCESS` \| `FAIL` 枚举值之一
| message {data-indent=1} | string | 业务处理附加信息
| acq_tid {data-required data-indent=1} | string | 受理网络交易流水号
| fix_recv_amount {data-indent=1} | integer | 收款金额，固定金额收款码收款金额，单位分
| mch_id {data-required data-indent=1} | string | 商户ID
| sub_mch_id {data-indent=1} | string | 特约商户ID，非服务商模式不填
| limit_bank_acc_type {data-required data-indent=1} | string | 限定付款方账户类型<br/>`UNLIMIT` \| `NO_CREDIT` 枚举值之一
| client_ip {data-required data-indent=1} | string | 服务器IP

{.im-table #response}

参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/institution/4019762388)
