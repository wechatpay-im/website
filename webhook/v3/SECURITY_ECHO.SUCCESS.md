---
title: 商户签名验签／加解密测试-回调接口(SECURITY_ECHO.SUCCESS)通知(JSON)
description: 商户调用商户签名验签／加解密测试接口后，如果请求中包含了notify_url参数，微信支付会在处理完成后，通过POST的请求方式向商户设置的回调地址发送回调通知，让商户验证回调通知的接收和处理能力。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`SECURITY_ECHO.SUCCESS` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`security` 枚举值
...
| echo_message {data-required data-indent=3}  | string | 回显消息
| encrypt_echo_message {data-required data-indent=3}  | string | 加密回显消息
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/merchant/4015164042) [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4019605946)
