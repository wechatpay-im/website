---
title: 商家名片会员用户积分兑券事件(BRAND_MEMBER_CARD_SP.POINT_EXCHANGE_COUPON)通知(JSON)
description: 当用户发起积分兑券时，微信会把相关事件和用户信息发送给商户。商户处理完成后，必须通过同步积分兑券结果接口，将处理结果同步给微信支付侧。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`BRAND_MEMBER_CARD_SP.POINT_EXCHANGE_COUPON` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`point_coupon` 枚举值
...
| record_id {data-required data-indent=3} | string | 兑券记录ID
...
| user_card_code {data-required data-indent=3} | string | 会员卡code
| exchange_coupon_template_id {data-required data-indent=3} | string | 积分兑券模板ID
| deduct_points {data-required data-indent=3} | number | 消耗积分数
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4015878622)
