...
| event_type {data-indent=1} | string | 通知的类型<br/>`HIRE_POWER_BANK.RECEIVE_INSURANCE` 枚举值
...
| order_id {data-indent=3} | string | 保险订单ID
| out_order_no {data-indent=3} | string | 商户保险订单号
| openid {data-indent=3} | string | 用户标识
| max_claim_count {data-indent=3} | number | 最大理赔次数
| claimed_count {data-indent=3} | number | 已理赔次数
| order_receive_time {data-indent=3} | string | 保险订单领取时间
| order_receive_state {data-indent=3} | string | 保险订单领取状态
