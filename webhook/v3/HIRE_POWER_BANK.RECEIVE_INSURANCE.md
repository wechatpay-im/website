---
title: 隔夜归还保险订单状态变化(HIRE_POWER_BANK.RECEIVE_INSURANCE)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`HIRE_POWER_BANK.RECEIVE_INSURANCE` 枚举值
...
| order_id {data-required data-indent=3} | string | 保险订单ID
| out_order_no {data-required data-indent=3} | string | 商户保险订单号
| openid {data-required data-indent=3} | string | 用户标识
| max_claim_count {data-required data-indent=3} | number | 最大理赔次数
| claimed_count {data-required data-indent=3} | number | 已理赔次数
| order_receive_time {data-required data-indent=3} | string | 保险订单领取时间
| order_receive_state {data-required data-indent=3} | string | 保险订单领取状态
| order_begin_time {data-indent=3} | string | 保险订单开始时间
| order_end_time {data-indent=3} | string | 保险订单结束时间
...
参阅 [官方文档](https://pay.weixin.qq.com/docs/merchant/apis/hire-power-bank-insurance/insurance-orders/status-change-notice.html)
