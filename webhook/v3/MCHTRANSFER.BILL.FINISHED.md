---
title: 商家转账回调(MCHTRANSFER.BILL.FINISHED)通知(用户确认模式)(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`MCHTRANSFER.BILL.FINISHED` 枚举值
...
| payment_method_type {data-indent=3} | string | 收款方式类型<br/>`CFT` | `WPHK` 枚举值之一
