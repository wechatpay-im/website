---
title: 商户交易拦截记录新增(BLOCKRECORD.CHANGE)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`BLOCKRECORD.CHANGE` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`block_record` 枚举值
...
| block_record_id {data-required data-indent=3} | string | 交易拦截记录ID
| block_count_level {data-required data-indent=3} | string | 商户管理记录状态<br/>`LESS_THAN_TWENTY` \| `LESS_THAN_ONE_HUNDRED` \| `LESS_THAN_ONE_THOUSAND` \| `OVER_ONE_THOUSAND` 枚举值之一
