---
title: 微信保单服务用户授权成功(INSURANCE_POLICY_MANAGEMENT.GRANTED)通知(JSON)
description: 用户使用微信保单服务，当用户授权上传保单后，微信支付会通过 POST 请求的方式，向商户预先设置的回调地址发送回调通知，让商户知晓用户已经授权成功。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`INSURANCE_POLICY_MANAGEMENT.GRANTED` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`insurance_policy_management` 枚举值
...
| out_insurance_no {data-required data-indent=3} | string | 商户保险编码
| insurance_name {data-required data-indent=3} | string | 保险名称
| auth_state {data-required data-indent=3} | string | 授权状态<br/>`AUTH_STATE_INIT` \| `AUTH_STATE_PENDING` \| `AUTH_STATE_GRANTED` \| `AUTH_STATE_DENIED` \| `AUTH_STATE_INEFFECTIE` 枚举值之一
| user_auth_time {data-required data-indent=3} | string | 用户授权时间
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/merchant/4016943540)
