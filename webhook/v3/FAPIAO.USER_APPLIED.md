...
| event_type {data-indent=1} | string | 通知的类型<br/>`FAPIAO.USER_APPLIED` 枚举值
...
| fapiao_apply_id {data-indent=3} | string | 发票申请单号，唯一标识一次开票行为。微信支付场景下，为微信支付订单号；非微信支付场景下，为调用获取抬头填写链接
| apply_time {data-indent=3} | string | 用户完成发票抬头填写的时间
