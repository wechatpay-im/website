---
title: 收付通合单代扣解约成功(ECOMMERCE_ENTRUST.TERMINATE)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`ECOMMERCE_ENTRUST.TERMINATE` 枚举值
...
| contract_termination_mode {data-required data-indent=3} | string | 协议解约方式<br/>`USER_TERMINATE` \| `MCH_API_TERMINATE` \| `WEPAY_WEB_TERMINATE` \| `CUSTOMER_SERVICE_TERMINATE` \| `SYSTEM_TERMINATE` 枚举值之一
| operate_time {data-required data-indent=3} | string | 操作时间
| attach {data-indent=3} | string | 附加数据
