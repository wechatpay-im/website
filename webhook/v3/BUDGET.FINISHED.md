---
title: 公益捐赠-预算申请完结回调(BUDGET.FINISHED)通知(JSON)
description: 用户使用公益捐赠服务，当预算锁定成功、关闭预算、超期关闭预算、解锁预算成功或预算资金消耗完时，微信支付会通过POST的请求方式，向商户预先设置的回调地址发送回调通知，让商户知晓预算锁定情况。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`BUDGET.FINISHED` 枚举值
...
| out_budget_no {data-required data-indent=3} | string | 商户预算单号
| budget_id {data-required data-indent=3} | string | 微信支付预算单号
| amount {data-required data-indent=3} | number | 预算总金额
| sponsor_mchid {data-required data-indent=3} | string | 出资商户号
| activity_name {data-required data-indent=3} | string | 活动名称
| activity_remark {data-required data-indent=3} | string | 活动说明
| state {data-required data-indent=3} | string | 预算单状态<br/>`FINISHED` 枚举值
| unlock_remark {data-indent=3} | string | 撤销说明
| finished_time {data-indent=3} | string | 完成时间
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4015555114)
