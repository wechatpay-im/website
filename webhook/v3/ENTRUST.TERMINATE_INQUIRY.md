---
title: 代扣解约问询(ENTRUST.TERMINATE_INQUIRY)通知(JSON)
description: 当用户解约时，委托代扣会通过该接口访问鸿蒙，这个协议是否可以解约。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`ENTRUST.TERMINATE_INQUIRY` 枚举值
...
| code {data-required data-indent=3} | string | 返回状态码
| messsage {data-required data-indent=3} | string | 返回信息
| mchid {data-required data-indent=3} | string | 商户号，微信支付分配的商户号
...
| out_user_code {data-required data-indent=3} | string | 商户侧用户标识
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/merchant/4012978455)
