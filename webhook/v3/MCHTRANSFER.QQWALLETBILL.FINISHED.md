---
title: 转账到QQ钱包(MCHTRANSFER.QQWALLETBILL.FINISHED)通知(JSON)
description: 商家转账到QQ钱包单据到终态后（转账完成或者转账失败，对应单据状态state的值为SUCCESS、CANCELLED和FAIL），微信支付会把单据的信息发送给商户，商户需要接收处理该消息，并返回应答。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`MCHTRANSFER.QQWALLETBILL.FINISHED` 枚举值
...
| transfer_bill_no {data-required data-indent=3} | string | 微信转账单号
| appid {data-required data-indent=3} | string | QQ互联开放平台AppID
...
| transfer_remark {data-required data-indent=3} | string | 转账备注
| openid {data-indent=3} | string | 收款用户OpenID
| user_qq {data-indent=3} | string | 收款用户QQ号
| user_name {data-indent=3} | string {data-tooltip=商户API证书对应的公钥加密后的BASE64字符串 data-encrypted=by-rsa-pubkey} | 收款用户姓名
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/merchant/4014301851)
