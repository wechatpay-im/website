---
title: 用户开通商家名片会员卡事件(MEMBERCARDSP.USER_CARD.CREATE)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`MEMBERCARDSP.USER_CARD.CREATE` 枚举值
...
| event_type {data-required data-indent=3} | string | 事件类型<br/>`MEMBERCARDSP.USER_CARD.CREATE` 枚举值
...
| card_color {data-indent=3} | string | 卡背景颜色
| card_picture_url {data-indent=3} | string | 卡背景图URL
| brand_id {data-required data-indent=3} | string | 品牌ID
| card_type {data-required data-indent=3} | string | 会员卡类型<br/>`PAY` \| `NORMAL` \| `BALANCE` 枚举值之一
| membership_number {data-required data-indent=3} | string | 会员卡编号
...
| values {data-indent=5} | string[] | 字段值
