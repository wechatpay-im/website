...
| account_number {data-required data-indent=3} | string | 入账银行账号后四位
| account_bank {data-required data-indent=3} | string | 入账银行
...
| reason {data-required data-indent=3} | string | 失败原因
| remark {data-required data-indent=3} | string | 提现备注
| bank_memo {data-required data-indent=3} | string | 银行备注
| account_type {data-required data-indent=3} | string | 出款账户类型<br/>`BASIC` \| `OPERATION` \| `FEES` 枚举值之一
| account_number {data-required data-indent=3} | string | 入账银行账号后四位
| account_bank {data-required data-indent=3} | string | 入账银行
| bank_name {data-indent=3} | string | 入账银行全称（含支行）
...
| status {data-required data-indent=3} | string | 提现单状态<br/>`CREATED` \| `PROCESSING` \| `FINISHED` \| `ABNORMAL` 枚举值之一
| withdraw_id {data-required data-indent=3} | string | 微信支付提现单号
| out_request_no {data-required data-indent=3} | string | 商户提现单号
| total_amount {data-required data-indent=3} | integer | 提现金额
| success_amount {data-indent=3} | integer | 提现成功金额
| fail_amount {data-indent=3} | integer | 提现失败金额
| refund_amount {data-indent=3} | integer | 提现退票金额
| create_time {data-required data-indent=3} | string | 发起提现时间
| update_time {data-required data-indent=3} | string | 提现状态更新时间
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4013049135)
