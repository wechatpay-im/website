---
title: 支付分服务用户授权成功(PAYSCORE.USER_OPEN_SERVICE)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`PAYSCORE.USER_OPEN_SERVICE` 枚举值
...
| contract_id {data-required data-indent=3} | string | 微信用户在校园轻松付下的合约的唯一标识，用户在校园轻松付小程序完成签约后，由微信支付生成。
