---
title: 商户交易拦截记录申诉(BLOCKSUBMISSION.CHANGE)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`BLOCKSUBMISSION.CHANGE` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`block_submisison_record` 枚举值
...
| appeal_record_id {data-required data-indent=3} | string | 交易拦截记录申诉ID
| appeal_result {data-required data-indent=3} | string | 商户管理记录状态<br/>`PASS` \| `REJECT` 枚举值之一
