---
title: 商家转账批次关闭回调(MCHTRANSFER.BATCH.CLOSED)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`MCHTRANSFER.BATCH.CLOSED` 枚举值
...
| batch_status {data-required data-indent=3} | string | 批次状态<br/>`WAIT_PAY` \| `ACCEPTED` \| `PROCESSING` \| `FINISHED` \| `CLOSED` 枚举值之一
| total_num {data-required data-indent=3} | number | 批次总笔数
| total_amount {data-required data-indent=3} | number | 批次总金额
| close_reason {data-required data-indent=3} | string | 批次关闭原因<br/>`CLOSED` \| `OVERDUE_CLOSE` \| `TRANSFER_SCENE_INVALID` 枚举值之一
