---
title: 用户确认自动收款授权(MCHTRANSFER.AUTHORIZATION.CONFIRMED)通知(用户确认模式)(JSON)
description: 微信支付系统将通过自动收款授权结果通知接口通知商户系统处理用户确认和关闭自动收款授权的结果。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`MCHTRANSFER.AUTHORIZATION.CONFIRMED` 枚举值
...
| out_authorization_no {data-required data-indent=3} | string | 商户侧授权单号
| appid {data-required data-indent=3} | string | 商户AppID
| openid {data-required data-indent=3} | string | 收款用户OpenID
| user_display_name {data-required data-indent=3} | string | 用户展示名称
| authorization_id {data-required data-indent=3} | string | 微信自动收款授权单号
| state {data-required data-indent=3} | string | 单据状态<br/>`TAKING_EFFECT` \| `CLOSED` 枚举值之一
| authorize_time {data-required data-indent=3} | string | 用户确认授权的时间
| close_info {data-indent=3} | object {data-tooltip="对应PHP的array"} | 自动收款授权的关闭信息
| close_time {data-required data-indent=4} | string | 关闭时间
| close_reason {data-required data-indent=4} | string | 自动收款授权的原因<br/>`CLOSE_VIA_MCH_API` \| `USER_CLOSE` 枚举值之一
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/merchant/4014512908)
