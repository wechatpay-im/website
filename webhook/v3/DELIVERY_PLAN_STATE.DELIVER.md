---
title: 投放计划状态变更(DELIVERY_PLAN_STATE.DELIVER)通知(JSON)
description: 投放计划创建后，如果设置了回调地址，当投放计划的状态或者审批状态发生变更时，会回调给品牌方。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`DELIVERY_PLAN_STATE.DELIVER` 枚举值
...
| plan_id {data-indent=3} | string | 投放计划ID
| previous_state {data-required data-indent=3} | string | 投放计划变更前状态<br/>`PLAN_CREATING` \| `PLAN_CREATED` \| `PLAN_TERMINATING` \| `PLAN_TERMINATED` \| `PLAN_EXPIRED` \| `PLAN_DELIVERING` \| `PLAN_PAUSED` 枚举值之一
| current_state {data-required data-indent=3} | string | 投放计划当前状态<br/>`PLAN_CREATING` \| `PLAN_CREATED` \| `PLAN_TERMINATING` \| `PLAN_TERMINATED` \| `PLAN_EXPIRED` \| `PLAN_DELIVERING` \| `PLAN_PAUSED` 枚举值之一
| previous_audit_state {data-required data-indent=3} | string | 投放计划变更前审核状态<br/>`AUDIT_INITIAL` \| `AUDIT_PROCESSING` \| `AUDIT_PASSED` \| `AUDIT_REJECTED` 枚举值之一
| current_audit_state {data-required data-indent=3} | string | 投放计划当前审核状态<br/>`AUDIT_INITIAL` \| `AUDIT_PROCESSING` \| `AUDIT_PASSED` \| `AUDIT_REJECTED` 枚举值之一
| change_reason {data-required data-indent=3} | string | 变更原因
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4016168699)
