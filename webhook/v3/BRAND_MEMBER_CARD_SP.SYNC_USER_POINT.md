---
title: 商家名片会员用户积分同步事件(BRAND_MEMBER_CARD_SP.SYNC_USER_POINT)通知(JSON)
description: 商户收到该事件通知后，需要通过同步积分余额接口，将用户积分余额同步给微信支付侧。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`BRAND_MEMBER_CARD_SP.SYNC_USER_POINT` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`sync_user_point` 枚举值
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4016096820)
