---
title: 用户管理会员卡事件(MEMBERCARD.USERCARD_MANAGE)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`MEMBERCARD.USERCARD_MANAGE` 枚举值
...
| event_type {data-required data-indent=3} | string | 事件类型<br/>`USER_VIEW_MEMBERCARD` \|`USER_DELETE_MEMBERCARD` \| `USER_MODIFY_INFORMATION` 枚举值之一
