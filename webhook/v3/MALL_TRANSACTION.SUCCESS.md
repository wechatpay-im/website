---
title: 商圈会员场内支付结果(MALL_TRANSACTION.SUCCESS)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`MALL_TRANSACTION.SUCCESS` 枚举值
...
| time_end {data-required data-indent=3} | string | 交易完成时间
| amount {data-required data-indent=3} | number | 用户实际消费金额，单位（分）
| transaction_id {data-required data-indent=3} | string | 微信支付订单号
| commit_tag {data-indent=3} | string | 手动提交积分标记，自动提交时无该字段，用于区分用户手动申请后推送的积分数据
