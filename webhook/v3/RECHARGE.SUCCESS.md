...
| event_type {data-indent=1} | string | 通知的类型<br/>`RECHARGE.SUCCESS` 枚举值
...
| account_type {data-indent=3} | string | 充值入账账户
| recharge_scene {data-indent=3} | string | 充值场景
| recharge_state {data-indent=3} | string | 充值状态
| recharge_state_desc {data-indent=3} | string | 充值状态描述
| recharge_amount {data-indent=3} | object | 充值金额
| amount {data-indent=4} | number | 总金额
| currency {data-indent=4} | string | 货币类型
| remark {data-indent=3} | string | 备注
| bank_transfer_info {data-indent=3} | object | 银行转账信息
| memo {data-indent=4} | string | 转账充值附言
| bill_no {data-indent=4} | string | 转入的银行流水单号
| bank_name {data-indent=4} | string | 开户银行名称
| bank_card_tail {data-indent=4} | string | 银行卡尾号
| qr_recharge_info {data-indent=3} | object | 扫码充值信息
| employee_type {data-indent=4} | string | 员工类型
| openid {data-indent=4} | string | 用户openid
| accept_time {data-indent=3} | string | 受理充值时间
...
参阅
- [官方文档](https://pay.weixin.qq.com/docs/partner/apis/platsolution-mch-recharge/notification/recharge-notice.html)
