---
title: 自动还款扣费还款成功(CREDIT_REPAYMENT.DEDUCTION_PAY)通知(JSON)
description: 微信支付通过还款成功回调通知，在还款成功后，微信支付会把相关支付信息发送给服务商。
---
...
> - 若商户应答回调接收失败，或超时(5s)未应答时，微信支付会按照（10秒/10秒/10秒/30秒/30秒/30秒/300秒/300秒/300秒/…/300秒）的频次重复发送回调通知，直至微信支付接收到商户应答成功，或达到最大发送次数（15次）
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`CREDIT_REPAYMENT.DEDUCTION_PAY` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`credit_repayment` 枚举值
...
| sub_mchid {data-required data-indent=3} | string | 子商户号
| appid {data-required data-indent=3} | string | 服务商AppID
| sub_appid {data-required data-indent=3} | string | 子商户AppID
| openid {data-required data-indent=3} | string | 用户标识
| order_state {data-required data-indent=3} | string | 还款单状态<br/>`DEDUCT_ORDER_STATE_PAY_SUCCESS` 枚举值之一
| out_trade_no {data-required data-indent=3} | string | 商户侧交易单号
| out_record_id {data-required data-indent=3} | string | 商户侧预约还款单号
| contract_id {data-required data-indent=3} | string | 还款协议ID
| deduct_amount {data-required data-indent=3} | string | 还款金额
| pay_success_amount {data-indent=3} | string | 实际还款金额
| description {data-required data-indent=3} | string | 产品描述
| attach {data-indent=3} | string | 商户数据包
| transaction_id {data-indent=3} | string | 微信支付的支付单号
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4016790592)
