---
title: 投放计划状态变更(DELIVERY_PLAN.CHANGE)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`DELIVERY_PLAN.CHANGE` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`delivery_plan` 枚举值
...
| plan_state {data-required data-indent=3} | string | 投放计划变更前状态<br/>`CREATED` \| `DELIVERING` \| `PLAN_TERMINATED` \| `PAUSED` \| `TERMINATED` \| `EXPIRED` 枚举值之一
| audit_state {data-required data-indent=3} | string | 投放计划当前审核状态<br/>`PROCESSING` \| `PASSED` \| `REJECTED` 枚举值之一
| change_reason {data-required data-indent=3} | string | 变更原因
| modify_time {data-required data-indent=3} | string | 修改时间
