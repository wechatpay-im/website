---
title: 用户签约状态变更(FACEPAY.USER_STATE_CHANGE)通知(JSON)
description: 刷脸用户在完成签约或解约后，微信会通知商户用户签约状态变更。商户需要接收处理，并返回应答。出于安全的考虑，我们对数据进行加密，商户需要先对通知数据进行解密，才能得到用户签约状态信息。
---
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`FACEPAY.USER_STATE_CHANGE` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`facepay` 枚举值
| algorithm {data-required data-indent=2} | string | 对数据进行加密的加密算法<br/>`AEAD_AES_256_GCM` 枚举值
| associated_data {data-required data-indent=2} | string | 数据加密的附加数据
...
| user_id {data-required data-indent=3} | string | 微信刷脸用户ID
| out_user_id {data-required data-indent=3} | string | 商户刷脸用户ID
| organization_id {data-required data-indent=3} | string | 机构编号
| mch_id {data-required data-indent=3} | string | 商户号
| notify_create_time {data-required data-indent=3} | string | 通知创建时间
| appid {data-required data-indent=3} | string | 微信APPID
| openid {data-required data-indent=3} | string | 微信用户openid
| contract_id {data-required data-indent=3} | string | 签约ID
...
参阅 [官方文档](https://pay.weixin.qq.com/wiki/doc/wxfacepay/develop/offline-brushing.html)
