---
title: 分账动账成功变动(PROFITSHARING.SUCCESS)通知(JSON)
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`PROFITSHARING.SUCCESS` \| `TRANSACTION.SUCCESS` 枚举值之一
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`profitsharing` 枚举值
...
| account {data-required data-indent=4} | string | 分账接收方的账号<br/>类型是`MERCHANT_ID`时，是商户号<br/>类型是`PERSONAL_OPENID`时，是个人OpenID
