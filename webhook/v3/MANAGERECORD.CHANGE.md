---
title: 商户管理记录(MANAGERECORD.CHANGE)通知(JSON)
description: 当商户名下的子商户新增商户管理记录/交易拦截记录，或者商户管理记录/拦截记录申诉状态发生变化时，微信支付会通过POST的请求方式，向商户预先设置的回调地址发送回调通知，让商户知晓子商户的商户管理或交易拦截记录变化。
---
...
> - 若商户应答回调接收失败，或超时(5s)未应答时，微信支付会按照（3s/60s/180s/600s/1200s/3600s）的频次重复发送回调通知，直至微信支付接收到商户应答成功，或达到最大发送次数（15次）
...
| event_type {data-required data-indent=1} | string | 通知的类型<br/>`MANAGERECORD.CHANGE` 枚举值
| resource_type {data-required data-indent=1} | string | 通知的资源数据类型
| summary {data-required data-indent=1} | string | 回调摘要
| resource {data-required data-indent=1} | object {data-tooltip="对应PHP的Array"} | 通知资源数据
| original_type {data-required data-indent=2} | string | 原始回调类型<br/>`manage_record` 枚举值
...
| manage_record_id {data-required data-indent=3} | string | 商户管理记录ID
| manage_record_state {data-required data-indent=3} | string | 商户管理记录状态<br/>`PENDING` \| `SUBMITTED` \| `EXPIRED` \| `UNDER_REVIEW` \| `RECOVERED` \| `REJECTED` 枚举值之一
...
参阅 [官方文档](https://pay.weixin.qq.com/doc/v3/partner/4014940982)
